            ...
            for img_name in os.listdir(person_path):
                ...
                if os.path.isfile(img_path):
                    ...
                    if results:
                        ...
                        predicted_label_encoded = model.predict([embedding])[0]
                        if predicted_label_encoded in encoder.classes_:
                            predicted_label = encoder.inverse_transform([predicted_label_encoded])[0]
                        else:
                            predicted_label = "Unknown"
                        
                        # Use angular distance for additional verification
                        predicted_label, confidence_score = predict_with_angular_distance(embedding, embeddings, labels, threshold=0.7)
                        
                        # Debugging: Print predictions and actual labels
                        print(f'Image: {img_path}, Actual: {person_name}, Predicted: {predicted_label}, Confidence: {1 - confidence_score}')
                        
                        # Check if prediction is correct and confidence is high
                        if predicted_label == person_name and (1 - confidence_score) > 0.7:
...
# Test the function with an image and evaluate accuracy
predict('data\\val\\adriana\\Adriana Lima59_178.jpg')
evaluate_accuracy('data\\val')
